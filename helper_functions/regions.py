"""
Discrete region identification functions for ShallowLandslider
...
from .utilities import progress_iter


# %% Calculate regions from binary grid
def calculate_regions(
    binary_grid,
    proximity_weight_on=False,
    density_weight_on=False,
    proximity_weight_val=0,
    density_weight_val=0,
    threshold_val=0,
    connect_val=4,
):
    """
    ...
    if proximity_weight_on:
        proximity_weights = _proximity_weighting(binary_grid)
        proximity_weights /= np.max(proximity_weights)  # Normalise weights

    if density_weight_on:
        density_weights = _gaussian_density_weighting(binary_grid)
    ...
    labeled_array, num_features = label(weighted_binary_grid, structure=struct_arr)

    return labeled_array, num_features


# %%% Helper functions for calculate_regions
def _proximity_weighting(binary_grid):
...
def _gaussian_density_weighting(binary_grid):
    return gaussian_filter(binary_grid.astype(float), sigma=1)


# %% Split regions by topographic aspect


def split_groups_by_aspect(
    groups,
    aspect_array,
    zones=None,
    min_size=2,
    ...
    pbar = progress_iter(np.unique(groups), verbose=verbose, desc="Splitting by aspect")
    ...
    if small_regions and handle_small == "remove":
        ...
        for region in small_regions:
            ...
            if len(neighbor_labels) > 0:
                ...
                neighbor_counts = [
                    (nl, np.sum(new_groups[neighbor_mask] == nl))
...
# %%% Helper functions for split_groups_by_aspect


def _create_zones(interval=20):
    """
...
def calculate_region_properties(
    ...
    if handle_small in ["merge", "remove"] and min_size > 1:
        ...
        working_labeled_array = handle_small_regions(
            working_labeled_array, min_size, handle_small, grid, verbose=verbose
        )
    else:
    ...
    calculate_topographic_stats(
        props,
        unique_labels,
        working_labeled_array,
        elevation_grid,
        slopes_grid,
        aspect_array,
    )
    ...
    calculate_geometric_properties(
        props, unique_labels, working_labeled_array, region_properties, grid
    )

    calculate_slope_length(props, unique_labels, working_labeled_array, grid)

    calculate_elevation_dir_dims(
        props,
        unique_labels,
        working_labeled_array,
        elevation_grid,
...
# %%% Helper functions for calculate_region_properties


def handle_small_regions(
    labeled_array, min_size, method="merge", grid=None, verbose: bool = False
):
    """
    ...
    for region in small_regions:
        ...
        if len(neighbor_labels) > 0:
            neighbor_counts = [
                (nl, np.sum(modified_array[neighbor_mask] == nl))
                for nl in neighbor_labels
            ]
            best_neighbor = max(neighbor_counts, key=lambda x: x[1])[0]
...
def calculate_topographic_stats(
    props, unique_labels, labeled_array, elevation_grid, slopes_grid, aspect_array
):
    ...
    props["min_elevation"] = labeled_comprehension(
...
def calculate_geometric_properties(
    props, unique_labels, labeled_array, region_properties, grid
):
    for i, label_num in enumerate(unique_labels):
        ...
        props["perimeter"][i] = _calculate_perimeter(region_mask, grid.dx, grid.dy)
        if props["perimeter"][i] > 0:
            props["compactness"][i] = (
                4 * np.pi * props["area"][i] / (props["perimeter"][i] ** 2)
            )

        region_idx = next(
            (j for j, r in enumerate(region_properties) if r.label == label_num), None
        )
        if region_idx is not None:
            _extract_region_shape_metrics(props, i, region_properties[region_idx], grid)


def _calculate_perimeter(region_mask, dx, dy):
...
def _extract_region_shape_metrics(props, i, region, grid):
...
def calculate_slope_length(props, unique_labels, labeled_array, grid):
    """
...
def calculate_elevation_dir_dims(
    props, unique_labels, labeled_array, elevation_grid, aspect_array, slopes_grid, grid
):
    """
    ...
    for i, label_num in enumerate(unique_labels):
        ...
        if gradient_direction is None:
            ...
            if np.sum(valid_mask) > 0:
                ...
                mean_aspect_radians = np.arctan2(
        ...
        props["slope_direction_length_new"][i] = np.max(gradient_projections) - np.min(
            gradient_projections
        )
        props["perpendicular_width_new"][i] = np.max(perp_projections) - np.min(
            perp_projections
        )
