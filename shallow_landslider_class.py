...
class ShallowLandslideSimulator:
    ...
    def _process_grid(self):
        ...
        pf = PriorityFloodFlowRouter(
            ...
            update_hill_depressions=self.config["flow_params"][
                "update_hill_depressions"
            ],
        ...
        if "soil__depth" not in self.grid.at_node:
            ...
            self.soil_depth = apply_soil_depth(
                ...
                relationship=self.config["soil_params"]["relationship"],
                # kwargs for various elevation-based soil depths
                decay_rate=self.config["soil_params"]["decay_rate"],
                exponent=self.config["soil_params"]["exponent"],
    ...
    def calculate_instability(self):
        ...
        self.acceleration_horizontal_array, self.acceleration_vertical_array = (
            generate_acceleration_grid(
                self.grid,
        ...
        self.factor_of_safety_vals = factor_of_safety(
            self.grid, cohesion_eff, angle_int_frict
        )
        if self.config["plot_intermediates"]["factor_of_safety"]:
            fos_plot_limit = 2.0
            self.plot_intermediate_maps(
                drape_variable=np.ma.masked_invalid(
                    np.ma.masked_greater(self.factor_of_safety_vals, fos_plot_limit)
                ),
        ...
        self.a_transient_zero, a_sliding_zero, a_diff_zero = (
            critical_transient_acceleration(
                self.grid,
                cohesion_eff,
                angle_int_frict,
        ...
        self.a_transient_EQ, self.a_sliding_EQ, self.a_diff_EQ = (
            critical_transient_acceleration(
                self.grid,
                cohesion_eff,
                angle_int_frict,
        ...
        if self.config["plot_intermediates"]["critical_acceleration"]:
            self.plot_intermediate_maps(
                drape_variable=np.ma.masked_invalid(
                    np.ma.masked_less(self.a_diff_EQ, 0)
                ),
        ...
        if self.config["plot_intermediates"]["unstable_areas"]:
            self.plot_intermediate_maps(
                drape_variable=np.ma.masked_invalid(self.sliding_locations_bool),
    ...
    def identify_failure_regions(self):
        ...
        self.labeled_array, num_features = calculate_regions(
            self.sliding_array_bool, connect_val=8
        )
        ...
        for label_number in labeled_groups:
            ...
            if label_number > 0:
                self.labeled_array_filled[
                    binary_fill_holes(masked_array, structure=struct)
                ] = label_number
    ...
    def filter_regions_by_aspect(self, split_by_width=None):
        ...
        aspect_intervals = create_zones(
            interval=self.config["simulation"]["aspect_interval"]
        )
        ...
        for label_number in split_aspect_groups:
            ...
            if label_number > 0:
                factor_of_safety_groups[masked_array] = np.nanmedian(
                    factor_of_safety_grid[masked_array]
                )
                a_transient_groups[masked_array] = np.nanmedian(
                    a_transient_grid[masked_array]
                )
            else:
        ...
        if split_by_width is not None:
            ...
            self.split_subgroups, split_info = recursive_split_wide_regions(
                ...
                aspect_array=self.aspect_nodes_array,
                slopes_grid=self.slopes_degrees,
            ...
            split_subgroup_props, self.split_subgroups = calculate_region_properties(
                self.grid,
        ...
        if self.config["plot_intermediates"]["filled_and_split"]:
            self.plot_intermediate_maps(
                drape_variable=np.ma.masked_less_equal(self.aspect_subgroups, 0),
    ...
    def select_potential_landslides(self):
        ...
        if selection_method == "probabilistic":
            ...
            self.selected_groups, selected_proportion = probabilistic_group_selection(
                ...
                random_seed=self.config["simulation"]["random_seed"],
                custom_proportion=self.config["simulation"]["custom_proportion"],
            )
            ...
            pga_weighted_probabilities, proportion, pga_meta = (
                generate_landslide_proportion_from_pga(
                    self.grid,
            ...
            self.pga_weighted_groups, pga_weighted_group_labels = (
                select_groups_by_proportion_weighted(
                    labeled_array=self.aspect_subgroups,
            ...
            pga_weighted_group_props, self.pga_weighted_groups = (
                calculate_region_properties(
                    self.grid,
    ...
    def calculate_displacements(self):
        ...
        """
        time_shaking = (
            np.ones_like(self.aspect_subgroups)
            * self.config["simulation"]["time_shaking"]
        )
        ...
        condition_array[self.newmark_displacement_select > displacement_threshold] = (
            True
        )
    ...
    def trace_sediment_paths(self):
        ...
        self.landslide_paths, landslide_proportions, self.path_details = (
            trace_paths_landslides(
                self.grid,
        ...
        self.transport_zones_grid = transport_zones_info["extended_zones"].reshape(
            self.grid.shape
        )

        transport_zone_props, self.transport_zones_grid = calculate_region_properties(
            grid=self.grid,
    ...
    def _create_transport_zones(self, delta_soil_depth):
        ...
        for start_node in self.node_ids:
            if start_node >= len(selected_groups_1d):
                print(
                    f"Warning: start_node {start_node} is out of bounds for selected_groups"
                )
                continue
        ...
        for region_id, start_nodes in nodes_by_region.items():
            ...
            for start_node in start_nodes:
                if start_node in self.path_details:
                    ...
                    for path, proportion in path_info_list:
                        valid_path_nodes = [
                            node for node in path if node < len(selected_groups_1d)
                        ]
            ...
            if len(transport_nodes) > 0:
                transport_mask = np.isin(
                    np.arange(len(selected_groups_1d)), transport_nodes
                )
            ...
            total_eroded = (
                np.sum(np.abs(delta_soil_depth[erosion_in_zone]))
                if np.any(erosion_in_zone)
                else 0.0
            )
            total_deposited = (
                np.sum(delta_soil_depth[deposition_in_zone])
                if np.any(deposition_in_zone)
                else 0.0
            )
            ...
            zone_details[region_id] = {
                ...
                "area_expansion_ratio": np.sum(zone_mask) / np.sum(original_region_mask)
                if np.sum(original_region_mask) > 0
                else 0,
            }
    ...
    def plot_intermediate_maps(
        ...
        if drape_variable is None:
            imshowhs_grid(
                self.grid,
                values=self.grid.at_node["topographic__elevation"],
                var_name="Elevation",
                var_units="m",
                ticks_km=True,
                cmap="terrain",
    ...
    def load_run(cls, file_name):
        ...
        def _reconstruct_grid(grid_arrays):
            grid = RasterModelGrid(
                (grid_arrays["n_rows"], grid_arrays["n_cols"]),
                xy_spacing=grid_arrays["dx"],
            )

            if grid_arrays["topographic_elevation"] is not None:
                grid.at_node["topographic__elevation"] = grid_arrays[
                    "topographic_elevation"
                ].copy()
            ...
            if grid_arrays["bedrock_elevation"] is not None:
                grid.at_node["bedrock__elevation"] = grid_arrays[
                    "bedrock_elevation"
                ].copy()
