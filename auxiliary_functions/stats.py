"""
Functions for statistics-based splitting and dimension calculation of landslide areas
...
def conditional_sample(
    ...
    if isinstance(kde_results, dict):
        if (
            ...
            and "by_category" in kde_results
            and category in kde_results["by_category"]
        ):
    ...
    if log_condition:
        if condition_value <= 0:
            raise ValueError(
                f"Cannot log-transform condition value {condition_value}: must be > 0"
            )
    ...
    while len(accepted_samples) < n_samples and attempts < max_attempts:
        ...
        if len(valid_responses) > 0:
            bounds_mask = (valid_responses >= return_bounds[0]) & (
                valid_responses <= return_bounds[1]
            )
    ...
    if len(accepted_samples) < n_samples:
        print(
            f"Warning: Only obtained {len(accepted_samples)} valid samples instead of {n_samples}"
        )
...
def plot_conditional_samples(
    ...
    plt.ylabel(y_col)
    plt.title(
        f"Conditional Sampling: P({'Y' if is_x_condition else 'X'}|{condition_var}){title_suffix}"
    )
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_multiple_conditional_samples(
    dataframe,
    x_col,
    y_col,
    kde_results,
