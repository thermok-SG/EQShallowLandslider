...
def split_groups_by_aspect(
    groups, aspect_array, zones=None, min_size=2, handle_small="merge"
):
    """
...
def calculate_region_properties(
    grid, labeled_array, slopes, aspect_array, min_size=1, handle_small="keep"
):
    """
    ...
    if handle_small in ["merge", "remove"] and min_size > 1:
        ...
        working_labeled_array = handle_small_regions(
            working_labeled_array, min_size, handle_small, grid
        )
    else:
...
def calculate_geometric_properties(
    ...
    for i, label_num in enumerate(unique_labels):
        ...
        props["perimeter"][i] = calculate_perimeter_vectorized(
            region_mask, grid.dx, grid.dy
        )
